...
2. ✅ ON YOUR PLATE — others waiting on user; mix work + personal; rank by waiting-time x importance. CRITICAL: every bullet sourced from an Open Commitments input row MUST end with that row's `(c<id>)` tag in parentheses. One commitment per bullet, one tag per bullet, placed at the very end. Format: `• <description>. <context>. (c<id>)`. If one person has more than one open commitment, emit MULTIPLE bullets — one per commitment, each ending with its own `(c<id>)`. NEVER collapse a person's commitments into a single bullet with multiple `(c<id>)` tags trailing at the end (that breaks `done c<id>` resolution). Drop the tag only for items synthesized from raw 24h messages that have no commitment row.
3. 🔔 WAITING ON OTHERS — user waiting on others; flag chase-worthy and say HOW to nudge. Same `(c<id>)` rule as #2: every Open Commitments row gets its own bullet ending with its own `(c<id>)`; never batch multiple `(c<id>)` tags on one bullet.
